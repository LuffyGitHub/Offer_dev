...
Page({
  data: {
    settings: { pushEnabled: true, pushTime: '08:00', showInRank: true, showProgress: true }
  },
  ...
  async saveToCloud() {
    try {
      await api.user.saveSettings(getOpenid(), this.data.settings)
      app.globalData.settings = this.data.settings
      set(STORAGE_KEYS.SETTINGS, this.data.settings)
    } catch (e) { console.warn('服务器保存设置失败', e) }
  },
  ...
  clearData() {
    wx.showModal({
      title: '确认清除',
      content: '清除所有本地数据后回到初始状态，服务器数据不受影响，确定吗？',
      success: (res) => {
        if (res.confirm) {
          try {
            wx.clearStorageSync()
